...
@export var step_floor_snap: float = 0.4
...
func _handle_controller_look(delta: float) -> void:
	...
	if abs(look_x) < controller_look_deadzone:
		look_x = 0.0

	if abs(look_y) < controller_look_deadzone:
		look_y = 0.0
	...
	head.rotate_x(-look_y * controller_look_sensitivity * delta)
